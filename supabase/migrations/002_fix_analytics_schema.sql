-- Fix analytics schema - drop and recreate if needed
...
CREATE POLICY "Allow anonymous inserts" ON voice_queries FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow anonymous inserts" ON page_views FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow anonymous inserts" ON engagement_events FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow anonymous inserts" ON error_logs FOR INSERT WITH CHECK (true);

-- 10. Materialized View for Daily Analytics
...
-- Success message
SELECT 'x444 Analytics Schema Fixed Successfully!' as message;
